...
func _ready() -> void:
	...
	var HealthBars = get_tree().get_nodes_in_group("HealthBarEnemy")
...
func chase_player() -> void:
	if global_position.distance_to(player.global_position) < dist:
		...
		animated_enemy.play("EnemyRanged_Shoot")
		_shoot()
		moving = true
		...
		moving = true
		animated_enemy.play("EnemyRanged_Run")
...
func SET_pause_or_end(state):
	pause_or_end = state
	animated_enemy.play("EnemyRanged_Idle")
	timer.stop()
